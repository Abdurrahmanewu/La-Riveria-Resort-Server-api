...
async function run() {
  try {
    ...
    app.post("/payments", async (req, res) => {
      ...
      app.post("/payments/success/:tranId", async (req, res) => {
        ...
        if (result.modifiedCount > 0) {
          ...
          res.redirect(
            `https://la-riveria-resort-client.vercel.app/dashboard/payments/success/${req.params.tranId}`
          );
        }
      ...
      app.post("/payments/fail/:tranId", async (req, res) => {
        ...
        if (result.deletedCount > 0) {
          res.redirect(
            `https://la-riveria-resort-client.vercel.app/dashboard/payments/fail/${req.params.tranId}`
          );
        }
        // console.log(req.params.tranId);
      });
    ...
    app.get("/reservations", async (req, res) => {
      let query = {};
      if (req.query.email) {
        query = { "paymentData.mail.email": req.query.email };
        const result = await finalBookingsCollection.find(query).toArray();
        res.send(result);
      } else {
        const result = await finalBookingsCollection.find().toArray();
        res.send(result);
      }
    });
